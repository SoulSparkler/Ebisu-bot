...
class Trader:
    ...
    def close_market(self, market_slug: str, winner: str, btc_start: float, btc_final: float) -> Optional[Dict]:
        ...
        # PnL
        estimated_fee = total_cost * 0.02  # 2% taker fee estimate
        gross_pnl = payout - total_cost
        pnl = gross_pnl - estimated_fee
        logger.info("REAL_PNL pnl=%.4f gross=%.4f fee_est=%.4f", pnl, gross_pnl, estimated_fee)
